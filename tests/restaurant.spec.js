...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  ...
  test(
    'TESTE 1: Verifique se o retorno da função createMenu() é um objeto que possui a chave fetchMenu, a qual tem como valor uma função.', () => {
      expect(typeof(createMenu(obj)).valueOf).toEqual('function')
  })
  test('TESTE 2: Verifique se \'objetoRetornado.fetchMenu()\' retorna um objeto cujas chaves são somente `food` e `drink`, considerando que a função createMenu() foi chamada com o objeto: `{ food: {}, drink: {} }`.', () => {
    const objetoRetornado = createMenu({ food: {}, drink: {} });
    expect(Object.keys(objetoRetornado.fetchMenu())).toEqual(['food', 'drink'])
  })
  test('TESTE 3: Verifique se o menu passado pra função createMenu é identico ao menu recuperado pela função \'objetoRetornado.fetchMenu\'', () => {
    const objetoRetornado = createMenu(obj);
    expect(createMenu(obj).fetchMenu()).toEqual(objetoRetornado.fetchMenu())
  })
  test('TESTE 4: Verifique se \'objetoRetornado.consumption\', após a criação do menu, retorna um array vazio.', () => {
    const objetoRetornado = createMenu(obj);
    expect(objetoRetornado.consumption).toEqual([])
  })
test('TESTE 5: Verifique se, ao chamar uma função associada à chave `order` no objeto retornado,  passando uma string como parâmetro (como `objetoRetornado.order(\'coxinha\')`), tal string é adicionada  ao array retornado em `objetoRetornado.consumption`.', () => { 
  const objetoRetornado = createMenu(obj);
  objetoRetornado.order('coxinha') 
  expect(objetoRetornado.consumption.find(elemento => elemento === 'coxinha')).toEqual('coxinha')
})
test('TESTE 6: Verifique se, ao adicionar três pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos.', () => {
    const objetoRetornado = createMenu(obj);
    objetoRetornado.order("coxinha");
     objetoRetornado.order("agua");
     objetoRetornado.order("sopa");
...
test('TESTE 7: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a consumption.', () => {
  const objetoRetornado = createMenu(obj);
  objetoRetornado.order('coxinha');
  objetoRetornado.order('agua');
  objetoRetornado.order('coxinha');
...
test('TESTE 8: Verifique se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`', () => {
  const objetoRetornado = createMenu(obj);
  objetoRetornado.order('coxinha');
  objetoRetornado.order('agua');
  objetoRetornado.order('coxinha');
